...
| X13 | **One authored text, two audiences** (surfaced by the staff notification centre, 2026-08-24): a B7 `notify` outcome carries `recipients` (patient / team / lead) but a single `notifyText`. With only the patient centre built, that was invisible; now that the team can read their copy, a rule written in patient voice ("your care team has been notified") arrives at the care team reading oddly. | **Decided (owner, 2026-08-25): option (a) — implemented.** The bundle schema carries per-audience texts (`notifyTexts` keyed patient/team/lead), with the pre-X13 single `notifyText` honoured as the fallback so already-published payloads keep delivering. The B7 editor shows one field per selected recipient; evaluation stores the notification body keyed by audience, and dispatch hands each recipient their own copy (a lead addressed as *lead* reads the lead text even when the whole team is also notified). The seeded severe-nausea rule now tells the patient the calm what-happens note and the lead a clinical review prompt. |
...
| X17 | **Wave-4 residuals that are product questions, not visual execution** (2026-08-25): (a) C4 draws a two-pane inbox (thread list beside the conversation) with day-separator chips and in-stream alert chips ("High alert from Weekly symptom survey — 08:12"); the build navigates list→thread and keeps alerts in the triage queue. (b) P12 draws a semantic summary ("Nausea — Severe; Everything else — Mild or none") that needs a rule for which answers count as notable; the build shows the honest full answer list. (c) P2's mobile menu draws unread badges, a locale switcher, Settings and a "Help & contact" entry — the last needs a help destination that does not exist. (d) P10 thread cards carry an author-prefixed preview ("Mikael: …") and a "Care team — oncology" specialty line; neither the author nor a specialty field rides the thread-list payload today. (e) P8 draws personal-information rows (address, phone) and time/date-format preferences the identity model does not store. | **Owner: "follow your prioritization" (2026-08-25).** **(a) delivered**: the staff surface is the two-pane inbox — searchable thread list beside the conversation, selection on the URL, collapsing to list→thread on small screens; the stream gained day-separator chips and severity-toned in-stream alert pills naming the survey, each opening the PP6 detail. **(d) delivered**: thread rows carry the last message's author (id, given name, realm) and, on the patient list, the treatment's care-team name — previews read "Mikael: …" / "You: …" and P10 shows "Care team — {team}". **(b) stays gated on clinical sign-off**; the concrete rule proposed for that sign-off: an answer is "notable" on P12 exactly when it fired a rule trigger on that response (the same mechanism that authors the patient's calm note), surfaced to the patient as their own answer only — never a severity. **(c) stays gated** on a help destination existing. **(e) corrected by the closing pass (2026-08-25)**: the patient record already stores phone and address — the staff profile shows and edits them — so the gap is only P8's self-view, a small work package (a self-read endpoint with its matrix action and audit), not a data-model change; time/date-format preferences remain non-existent. |
| X18 | **The closing verification pass (2026-08-25)**: every board not previously compared side-by-side (B-series, T2–T4, PP1/PP3–PP6, A2–A5, C2/C3/C6/C7, remaining L and S boards) was checked against live screenshots. Most align: A3/A5, B1/B2 (the builder's uppercase panel idiom matches the canvas), C6's state components, PP1/PP6, T3/T4 dialogs, C7's per-answer rules verdicts and body map. Two idiom gaps were carried in this pass: the kit's CardHeader now titles every card in the serif display voice (profile, treatment detail, values/symptoms/responses cards), and the patient nav counts due surveys (P7), sharing the landing's query key so no new disclosure. The rest are FEATURE-level differences: (i) C2 draws a richer patient summary (inline alert banner with actions, three-series values columns, latest-activity feed, Message/New task header buttons) than the build's overview; (ii) C3 draws the roster as a worklist table (alert/survey-status/next-activity columns, My patients/Whole team pills, alert-sorted) where the build lists name+age; (iii) T2 draws a Running/Templates toggle, an attached-surveys column and template authoring ("+ New template", Edit) — no template editor exists; (iv) C7's rules-summary card and alert comments live on PP6 in the build, deliberately; (v) P4's Back/Save-&-exit sit below the card with a continuous progress bar, and the category eyebrow has no backing section field in the survey schema; (vi) P11 draws circled icon tiles where the build uses bare icons; (vii) P12's Done/"Review my answers" pair is not drawn — the build shows the full answers inline; (viii) C5's "completed tasks move to the treatment's activity log" footer is omitted because the build has no such log surface. | **Owner (2026-08-26): implement (iv)–(vii), remove (viii) from scope for good, sit on (i)–(iii). Done.** (iv): the response view carries the canvas's "Alert rules of this program" card — the effective rule set (program overrides applied and marked) in plain language grouped by what a firing raises, with the builder link for authors — and, when the response raised an alert, the alert's comment trail with an in-place composer posting to the same append-only PP6 thread. (v): the survey schema gained per-page section names (`pageTitles` in the bundle, authored in the builder per locale, kept in the patient payload); P4 shows them as the category eyebrow, the Back and Save-&-exit controls moved to the canvas's top bar with a full-width Continue below, and the progress bar is segmented per question (the answered-based count stays — WP-14's deliberate visibility-aware semantics). (vi): P11's icons sit in the canvas's circled tiles, tinted while unread. (vii): P12 leads with a full-width Done and keeps the answer list behind "Review my answers". (viii) is out of scope permanently — the build has no treatment activity log for completed tasks to move to, so the canvas drops the line at next sync. (i)–(iii) remain with the owner. |
...
| X16 | **Systemic visual drift from the canvases** (owner finding, 2026-08-25): comparing the running product broadly against the handoff canvases, the owner judged the build "a lot off" the target. A board-by-board spot audit (P1, P6, C1, A1, L1 rendered from the canvases vs live screenshots) confirms the pattern: the structure and information of every screen match, and the recorded X-deltas (X10 icons, X11, X12, X14, X15) are intentional — but the design's visual execution is only partially carried. Recurring gaps: serif landing greetings with date lines (P1/C1) missing; thread bubbles built as tints instead of the design's solid-teal own-messages with author-meta below and the composer's toolbar-below/round-send arrangement (P6/C4); A1 built as a card list where the canvas draws a table with role chips, text-link actions and the reset explainer; C1 missing the "All X" card links, inline acknowledge/claim actions and the My patients/Whole team control; P1 missing date tiles, labeled card links and the Start button; L-series details (Show password, top-right locale switcher). The patient boards are also drawn mobile-first while the build is desktop-first. Technical note: the canvases render as plain HTML once `support.js` is stripped, so pixel-level reconciliation can be driven board by board. | **Decided (owner, 2026-08-25): option (b) — waves.** Wave 1 implemented: time-of-day serif greetings with date lines on P1 and C1; labeled card action links and the Start button on P1; the P6/C4 thread treatment (solid-teal own bubbles, author-and-time meta below, avatars for the other party) and the composer rearranged to the canvas (text above, tools below, round send); response views render the marked body map read-only. Wave 2 implemented: A1 rebuilt as the canvas's table (role chips with the high-privilege tints, quiet status text, text-link actions, the reset explainer box — status shows no deactivation date, the identity API does not expose one); C1 alert rows carry inline Acknowledge/Resolve, the tasks card lists the queue's next unclaimed tasks with inline Claim, and the agenda/messages cards gained labeled action links (the alerts and overdue cards already render their complete lists and link nowhere, and the overdue card has no staff survey-status page to link to); P1's Upcoming rows carry the date tiles. Note on the C1 segmented control: the canvas draws "My patients / Whole team", but the triage queue's base scope already IS the caller's care patients (and in the team-centric care model those coincide with the team's), so the build keeps WP-27's truthful "Whole team / Assigned to me" — the only narrower predicate that exists. Wave 3 implemented: the password fields (L1 sign-in, L3 set-password, reset) carry the canvas's in-field Show/Hide toggle; the pre-auth locale switcher already sat top-right as drawn. The mobile pass verified the patient boards at 390px — landing, thread and composer stack cleanly (the earlier mobile-nav and responsive-grid work already carried it) — with no code change needed. All three waves closed 2026-08-25. **Wave 4 (2026-08-25): the full-board sweep.** Every remaining board rendered and compared. Carried: serif italic card titles across the landing widgets, dashboard cards and tasks card (the canvases' one recurring typographic signature still missing); P7's asymmetric landing layout (act-now column wide, Upcoming beside); P3's due language (amber "Due today", red counted "N days overdue", plain future dates), the save-as-you-go reassurance line, and View links from submitted rows into P12; P4 asks in the serif voice; P12's check sits in the tinted circle; P9's survey rows became actionable (Start on open occurrences, future ones stay inert); P11 groups into Today and Earlier. As-built differences kept deliberately: P9 groups per day with a time column (finer than the canvas's TODAY/NEXT WEEK/month tiles), C5's grouping already matched, C4's thread treatment had carried over from wave 1, and question titles render in the bundled italic Newsreader (WP-03's single-cut display voice) where the canvas's P4 draws the upright cut. Product-shaped residuals moved to X17. |
