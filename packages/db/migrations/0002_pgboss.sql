-- 0002 pg-boss schema, version 37 (pg-boss 12.27.0).
--
-- Generated with pg-boss getConstructionPlans('pgboss') and checked in as a
-- reviewed migration: the worker role must not hold CREATE on the database,
-- so pg-boss never installs its own schema - both buses run migrate:false
-- and packages/db/src/jobs.ts asserts the version at startup.
--
-- Upgrading pg-boss: generate getMigrationPlans('pgboss', <from>) into a NEW
-- migration and bump EXPECTED_PGBOSS_VERSION in jobs.ts in the same PR.
-- pg-boss's own BEGIN/COMMIT/lock preamble is stripped - the migration
-- runner supplies the transaction and the advisory lock.
...
-- Queue privileges (ADR-0008): the worker consumes and maintains; the app
-- enqueues - which is what makes the transactional outbox one INSERT away
-- from domain writes. Nobody else sees the queue.
GRANT USAGE ON SCHEMA pgboss TO mio_app, mio_worker;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA pgboss TO mio_worker;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA pgboss TO mio_worker;
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA pgboss TO mio_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA pgboss TO mio_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO mio_worker;
ALTER DEFAULT PRIVILEGES IN SCHEMA pgboss
  GRANT SELECT, INSERT, UPDATE ON TABLES TO mio_app;
